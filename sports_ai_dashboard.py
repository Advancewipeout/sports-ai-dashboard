...
# 🔄 THE NATIVE STREAMLIT LIVE SYNC TRIGGER (High-Speed Local 1-second fragment loop)
...
def render_live_sports_matrix():
    ...
    if live_layer_df.empty:
        ...
        display_cols = [c for c in ["Sport", "Matchup", "Time Metric", "Score Ticker", "Odds Line", "Edge Margin %", "AI Action Directive", "Pick Team"] if c in live_layer_df.columns]
    ...
    if upcoming_layer_df.empty:
        ...
        display_cols = [c for c in ["Sport", "Matchup", "Odds Line", "Edge Margin %", "AI Action Directive", "Pick Team"] if c in upcoming_layer_df.columns]
